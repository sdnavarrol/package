...
module_param(SYSCLKDIV, uint, S_IRUGO);
module_param(BCLKDIV, uint, S_IRUGO);
module_param(DACLKDIV, uint, S_IRUGO);
module_param(ADCLKDIV, uint, S_IRUGO);
				...
				struct snd_pcm_hw_params *params)
{
	...
	//printk("************** ALU: codec_clock_hwparams ***************** \n");
	...
	// CBM = codec bclk master  - CFM = codec frame master
	if(!rtd->slave_en){
		//printk("************** ALU: codec_clock_hwparams - set dai_fmt for CBS_CFS ***************** \n");
		snd_soc_dai_set_fmt(codec_dai,SND_SOC_DAIFMT_CBS_CFS|SND_SOC_DAIFMT_I2S|SND_SOC_DAIFMT_NB_NF);
	}else{
		//printk("************** ALU: codec_clock_hwparams - set dai_fmt for CBM_CFM ***************** \n");
	...
	//printk("************** ALU: codec_clock_hwparams - CONFIG_SND_SOC_WM8960 ***************** \n");
	#if defined(CONFIG_I2S_MCLK_12MHZ)
		pTable = i2s_codec_12Mhz;
		data = pTable[index];
		//printk("************** ALU: codec_clock_hwparams - i2s_codec_12Mhz ***************** \n");
	#else
		//printk("************** ALU: codec_clock_hwparams - i2s_codec_12p288Mhz ***************** \n");
